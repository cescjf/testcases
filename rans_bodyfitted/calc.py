...
for index_angle in range(1,NUMANGLES+1):
    resultfile = open('results/sim_'+str(index_angle)+'.csv','w')
    resultfile.write("absvar-value,dLx,dLy\n")
    for index_perturb in range(1,NUMPERTURB+1):
        simname = "sim_"+str(index_angle)+"_"+str(index_perturb)
        print(simname)
...
for index_angle in range(1,NUMANGLES+1):
    simname="anasim_"+str(index_angle)
...
# for simname in sorted(os.listdir('.')): #sorted is needed since the reults might be written out of order
    # if simname[0:6]=='anasim':
      # for method in ['direct','adjoint']:
...
# for simname in sorted(os.listdir('.')): #sorted is needed since the reults might be written out of order
    # if simname[0:3]=='sim':
        # #reading the vlaue that was used for the abstract variable
        # print(simname)
        # with open(simname+'/sdesign/naca_plus.sdesign') as f:
            # lines=f.readlines()
            # index1=lines.index('ABSVAR\n')
        ...
        # #writing the result
        # resultfile = open('results/'+simname+'.csv','w')
        # resultfile.write("absvar-value,dLx,dLy\n")
        # resultfile.write(writeline)
        # resultfile.write("\n")
        # resultfile.close()
